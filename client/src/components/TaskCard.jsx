            ...
            tasks?.map((task) => (
              <div className="p-4 lg:w-1/3" key={task?._id}>
                <div className="h-full bg-gray-100 bg-opacity-75 px-8 py-4 rounded-lg overflow-hidden text-center relative">
                  ...
                  <p className="leading-relaxed text-gray-700 mb-3 overflow-hidden overflow-ellipsis max-h-16">
                  ...
                  {user.data.user._id == channel.creator ? (
                    ...
                    <>
                      {channel?.users?.some((channelUser) => {
                        return (
                          channelUser?.user == user.data?.user?._id &&
                          channelUser.status != "approved"
                        );
                      }) && (
                        <>
                          <UpdateTaskModal
